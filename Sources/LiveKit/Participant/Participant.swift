    ...
    init(room: Room, sid: Sid? = nil, identity: Identity? = nil) {
        ...
        _state.onDidMutate = { [weak self] newState, oldState in
            ...
            if newState.isSpeaking != oldState.isSpeaking {
                self.delegates.notifyQueue(label: { "participant.didUpdate isSpeaking: \(self.isSpeaking)" }) {
            ...
            {
                self.delegates.notifyQueue(label: { "participant.didUpdate metadata: \(newMetadata)" }) {
                    $0.participant?(self, didUpdateMetadata: newMetadata)
                }
                room.delegates.notifyQueue(label: { "room.didUpdate metadata: \(newMetadata)" }) {
            ...
            if let newName = newState.name, newName != oldState.name {
                ...
                self.delegates.notifyQueue(label: { "participant.didUpdateName: \(String(describing: newName))" }) {
                    $0.participant?(self, didUpdateName: newName)
                }
                // notify room delegates
                room.delegates.notifyQueue(label: { "room.didUpdateName: \(String(describing: newName))" }) {
                    $0.room?(room, participant: self, didUpdateName: newName)
                }
            }

            if newState.connectionQuality != oldState.connectionQuality {
                self.delegates.notifyQueue(label: { "participant.didUpdate connectionQuality: \(self.connectionQuality)" }) {
                    $0.participant?(self, didUpdateConnectionQuality: self.connectionQuality)
                }
                room.delegates.notifyQueue(label: { "room.didUpdate connectionQuality: \(self.connectionQuality)" }) {
