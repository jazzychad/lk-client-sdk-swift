...
public class Track: NSObject, Loggable {
         ...
         reportStatistics: Bool)
    {
        ...
        _state.onDidMutate = { [weak self] newState, oldState in
            ...
            {
                self.delegates.notifyQueue { $0.track?(self, didUpdateStatistics: statistics, simulcastStatistics: newState.simulcastStatistics) }
            }
        }
    }
             ...
             shouldSendSignal: Bool = false)
    {
        ...
        if _notify {
            delegates.notifyQueue(label: { "track.didUpdateIsMuted: \(newValue)" }) { delegate in
...
extension Track {
    ...
    func set(dimensions newValue: Dimensions?) -> Bool {
        ...
        delegates.notifyQueue(label: { "track.didUpdateDimensions: \(newValue == nil ? "nil" : String(describing: newValue))" }) {
